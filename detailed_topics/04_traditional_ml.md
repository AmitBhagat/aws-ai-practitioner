...
*   **Supervised Learning:** The model is trained on **labeled data** (inputs $X$ paired with correct output labels $Y$).
    ...
    *   *Analogy:* Studying for a test using flashcards that have the question on the front and the answer on the back. The model looks at features ($X$), makes a guess ($\hat{Y}$), checks the true answer ($Y$), and adjusts itself to minimize the error.  <br /> 🔍 **Example:** Training a classifier on historical customer records to predict customer churn ($Y \in \{0, 1\}$).
> [!WARNING]
...
*   **Unsupervised Learning:** The model is trained on **unlabeled data** to find hidden structures, patterns, or groupings within the features $X$.
    ...
    *   *Analogy:* Being handed a giant pile of mixed coins and sorting them into groups by size and color without knowing what they are. You group them purely by similarity.  <br /> 🔍 **Example:** Segmenting a customer database into distinct purchasing groups based on transactional features.
> [!WARNING]
...
*   **Reinforcement Learning:** An **agent** interacts with an **environment** by taking **actions** ($a_t$) and learning via trial-and-error feedback loops of **rewards** ($r_t$) and **penalties**.
    ...
    *   *Analogy:* Training a dog. There is no historical dataset. Instead, the dog performs actions, and you give treats (rewards) for good behavior or ignore/time-out (penalties) for bad behavior.  <br /> 🔍 **Example:** Directing an autonomous warehouse robot to navigate a floor grid.
> [!WARNING]
