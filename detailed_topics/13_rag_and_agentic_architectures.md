...
flowchart TD
    ...
    classDef store fill:#0f172a,stroke:#0ea5e9,color:#f8fafc,stroke-width:2px;
    classDef generate fill:#064e3b,stroke:#10b981,color:#f8fafc,stroke-width:2px;

    subgraph Ingestion["📥 Data Ingestion Phase"]
        Raw["📄 Raw Documents (S3)"]:::source --> Chunk["✂️ Document Chunking"]:::process
        Chunk --> Embed["🧮 Embedding Model (Titan)"]:::process
        Embed --> VDB["🗄️ Vector Store (OpenSearch)"]:::store
    end

    subgraph Retrieval["🔍 Retrieval & Generation Phase"]
        Prompt["✍️ User Prompt"]:::source --> VecQuery["🔢 Query Vectorizer"]:::process
        VecQuery --> Search["🔎 Vector Search (k-NN)"]:::process
        VDB --> Search
        Search --> Context["📦 Context Assembly<br/>(Prompt + Chunks)"]:::process
        Context --> LLM["🤖 LLM Context Window"]:::generate
    end
```
...
> [!WARNING]
> **Fixed-Size Chunking Overlap Gotcha:** If you set chunk size too small or overlap too small, semantic concepts (like numbers, price quotes, or names) get cut in half at boundaries, destroying their embedding similarity scores. Keep overlap balanced (typically 10-20% of chunk size).
...
> [!WARNING]
> **Vector Embedding Dimensionality Gotcha:** When storing vectors in pgvector or OpenSearch, the database index dimensions must *exactly* match the output dimensions of your embedding model (e.g. Titan Text Embeddings has 1536 dimensions; mismatching causes immediate failures).

> [!WARNING]
> **Bedrock Knowledge Base OpenSearch Orphanage:** Deleting a Bedrock Knowledge Base does NOT delete the underlying vector database collection (like OpenSearch Serverless). You must delete the collection manually in the OpenSearch console to stop persistent hourly billing.
...
```mermaid
graph TD
    classDef query fill:#1e293b,stroke:#475569,color:#f8fafc,stroke-width:2px;
    classDef agent fill:#1e1b4b,stroke:#6366f1,color:#f8fafc,stroke-width:2px;
    classDef lambda fill:#881337,stroke:#f43f5e,color:#f8fafc,stroke-width:2px;
    classDef db fill:#064e3b,stroke:#10b981,color:#f8fafc,stroke-width:2px;

    U["❓ User Query"]:::query --> A["🤖 Bedrock Agent (ReAct Loop)"]:::agent
