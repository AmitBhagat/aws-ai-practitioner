...
```mermaid
flowchart TD
    classDef input fill:#1e293b,stroke:#3b82f6,color:#f8fafc,stroke-width:2px;
    classDef wrong fill:#be123c,stroke:#f43f5e,color:#f8fafc,stroke-width:2px;
    classDef right fill:#047857,stroke:#10b981,color:#f8fafc,stroke-width:2px;
    classDef process fill:#1e1b4b,stroke:#6366f1,color:#f8fafc,stroke-width:2px;

    subgraph Standard["❌ Standard Prompting Flow"]
        SP["Standard Prompt<br/>(Direct Question)"]:::input --> S_Math["🤖 LLM Direct Prediction<br/>(No scratchpad)"]:::process
        S_Math --> WA["❌ Incorrect Answer<br/>(Symbolic failure)"]:::wrong
    end

    subgraph CoT["✅ Chain-of-Thought (CoT) Flow"]
        CP["CoT Prompt<br/>(Question + 'Think step-by-step')"]:::input --> R1["🧠 Step 1: Parse Variables"]:::process
        R1 --> R2["🧠 Step 2: Intermediate Math"]:::process
        R2 --> R3["🧠 Step 3: Sanity Check"]:::process
        R3 --> CA["✅ Correct Answer<br/>(Conditioned on reasoning history)"]:::right
    end

    %% Force vertical stacking by linking Standard to CoT invisibly
    WA -.->|" "| CP
    linkStyle 6 stroke:none,stroke-width:0px,color:transparent;
```
...
```
        By writing out its intermediate reasoning steps, the model actively builds a logical context history. The final answer token is then conditioned on this correct logical history, raising prediction accuracy.  <br /> 🔍 **Example:** 
        ```text
...
```
    where $\mathbb{I}$ is the indicator function evaluating to $1$ if the path's answer $A_i$ matches candidate answer $A$, and $0$ otherwise.  <br /> 🔍 **Example:** Generating 5 different Chain-of-Thought runs for a math puzzle. Run 1 yields `14`, Run 2 yields `12`, Run 3 yields `14`, Run 4 yields `14`, and Run 5 yields `14`. The self-consistency algorithm registers a $4/5$ majority vote for `14`, successfully filtering out the reasoning error in Run 2.
...
```mermaid
graph TD
    classDef question fill:#1e293b,stroke:#475569,color:#f8fafc,stroke-width:2px;
    classDef thought fill:#1e1b4b,stroke:#6366f1,color:#f8fafc,stroke-width:2px;
    classDef action fill:#0f172a,stroke:#0ea5e9,color:#f8fafc,stroke-width:2px;
    classDef observation fill:#be123c,stroke:#f43f5e,color:#f8fafc,stroke-width:2px;
    classDef answer fill:#064e3b,stroke:#10b981,color:#f8fafc,stroke-width:2px;

    Q["❓ User Question"]:::question --> T["🧠 Thought: Reason about state"]:::thought
    T --> A["🛠️ Action: Call API / Tool"]:::action
    A --> O["👁️ Observation: Tool Output"]:::observation
    O --> T2["🧠 Thought: Analyze observation"]:::thought
    T2 -- "Needs more data?" --> A
    T2 -- "Solved?" --> Ans["🎯 Final Answer to User"]:::answer
```
...
4.  **Loop:** The model evaluates the observation. If it has sufficient information, it generates the final answer; if not, it continues the loop with another thought-action cycle.  <br /> 🔍 **Example:** 
...
*   **XML Delimiters:** Standardizing the use of XML tags (e.g., `<context>`, `<rules>`, `<user_input>`) is highly recommended by top model developers (including Anthropic). Transformers are pre-trained on structured web documents, making them highly proficient at isolating text blocks wrapped in tags.  <br /> 🔍 **Example:** 
    ```text
[System Prompt]
