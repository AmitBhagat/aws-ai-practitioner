...
*   **Supervised Learning:** The model is trained on **labeled data** (inputs $X$ paired with correct output labels $Y$).
    ...
    *   **Exam Scenario:** A financial institution wants to build an automated application to predict whether loan applicants are likely to default on their payments. They compile a historical training dataset where features ($X$) like credit score, annual income, and debt-to-income ratio are paired with explicit binary target labels ($Y \in \{0, 1\}$ representing default status). The algorithm updates its weights to minimize the discrepancy between predictions ($\hat{Y}$) and true labels ($Y$).
> [!WARNING]
...
*   **Unsupervised Learning:** The model is trained on **unlabeled data** to find hidden structures, patterns, or groupings within the features $X$.
    ...
    *   **Exam Scenario:** An e-commerce business has accumulated petabytes of unlabeled customer interaction histories. The marketing team wants to segment these users into distinct buyer profiles to design targeted ad campaigns. They utilize an unsupervised **clustering** algorithm to automatically group customers into natural categories based on purchase frequency, average order value, and browsing patterns without pre-defined category labels.
> [!WARNING]
...
*   **Reinforcement Learning:** An **agent** interacts with an **environment** by taking **actions** ($a_t$) and learning via trial-and-error feedback loops of **rewards** ($r_t$) and **penalties**.
    ...
    *   **Exam Scenario:** A shipping logistics firm is designing a collision-avoidance system for autonomous forklifts in a crowded warehouse. The development team uses reinforcement learning to train the control policy ($\pi$). Instead of learning from a static historical dataset, the forklift simulator dynamically runs navigation loops, receiving numeric rewards for successfully reaching target shelves and heavy penalties (negative rewards) for getting stuck or colliding with walls.
> [!WARNING]
...
*   **Semi-supervised Learning:** Combines a small amount of labeled data with a large amount of unlabeled data to improve training efficiency and reduce manual labeling costs.  <br /> 🔍 **Example:** A photo tagging app has 1 million user photos but only 10,000 are labeled with person tags. It uses the 10,000 labeled photos to find similar clusters in the 990,000 unlabeled photos, labeling them semi-automatically.
> [!WARNING]
...
*   **Self-supervised Learning:** The model automatically generates its own labels from the input data (often by masking parts of the input and training the model to predict the missing pieces).  <br /> 🔍 **Example:** Pre-training a language model by masking $15\%$ of the words in a corpus (e.g., `"The [MASK] chased the mouse"`) and predicting `"cat"`. This is how foundation models (FMs) learn language syntax.
> [!WARNING]
...
*   **Multi-instance Learning:** Training examples are grouped into "bags", and labels are applied to the bags rather than individual instances.  <br /> 🔍 **Example:** Scanning a medical slide of cells (a bag). The slide is labeled `"malignant"` if at least one cell on the slide is cancerous. The individual cells themselves are unlabeled.
> [!NOTE]
...
*   **Inductive Inference:** Drawing generalized rules from specific observations.  <br /> 🔍 **Example:** Training a model on $100,000$ historical housing sales to estimate price functions based on square footage, then using that general function to estimate the price of any new house.
> [!WARNING]
...
*   **Deductive Inference:** Applying general rules or premises to determine specific logical outcomes.  <br /> 🔍 **Example:** A rule-based expert system that asserts `"All humans are mortal"` and `"Socrates is human"`, and logically deduces `"Socrates is mortal"`.
> [!WARNING]
...
*   **Transductive Inference:** Predicting specific outcomes for a specific target test set directly from training instances, without forming a generalized model first.  <br /> 🔍 **Example:** Graph-based semi-supervised learning where you propagate label values from a small set of labeled nodes directly to a set of unlabeled nodes on a fixed social network graph.
> [!NOTE]
...
*   **Multitask Learning:** Training a single model to perform multiple related tasks simultaneously, sharing early representation layers.  <br /> 🔍 **Example:** A self-driving vehicle perception model that simultaneously predicts lane lines (regression), detects pedestrians (object detection), and reads speed limit signs (classification) from a single camera feed.
> [!NOTE]
...
*   **Active Learning:** The model dynamically queries a human annotator (oracle) to label specific data points that lie close to its decision boundary to maximize training efficiency.  <br /> 🔍 **Example:** A document classification model scanning millions of emails. It selects only the 100 most ambiguous emails (where prediction probability is close to $0.5$) and requests labels from a human expert, ignoring the highly confident emails.
> [!WARNING]
...
*   **Online Learning:** Continuously updating the model's weights in real-time as new data streams in, rather than training in batches on a static dataset.  <br /> 🔍 **Example:** A search engine ranking algorithm adjusting its weights dynamically based on live click-through logs of active web users.
> [!WARNING]
...
*   **Transfer Learning:** Taking a model pre-trained on a generic, large dataset and using its parameters as a starting point to train on a smaller, domain-specific task.  <br /> 🔍 **Example:** Fine-tuning a ResNet model pre-trained on 1 million ImageNet images to classify rare skin lesions using only 500 clinical photos.
> [!WARNING]
...
*   **Ensemble Methods:** Combining predictions of multiple models to produce a single, more robust output.
    *   **Bagging (Bootstrap Aggregating):** Training multiple models in parallel on random subsets of the data (sampled with replacement) and averaging their predictions (or voting).  <br /> 🔍 **Example:** A Random Forest classifier composed of 100 decision trees, each trained on a bootstrap sample of user churn data.
    ...
    *   **Boosting:** Training models sequentially, where each new model is trained to focus on and correct the errors made by the previous models.  <br /> 🔍 **Example:** An AdaBoost or XGBoost regressor where each subsequent tree fits the residual errors of the prior ensemble.
        *   *Gotcha:* Boosting reduces both bias and variance but is highly sensitive to noisy outliers or mislabeled training points.
    *   **Stacking:** Training multiple different model architectures in parallel, and combining their outputs using another meta-learner model.  <br /> 🔍 **Example:** Passing predictions from an SVM and a Random Forest into a final Logistic Regression model to make the final classification.
...
```mermaid
graph TD
    classDef agent fill:#1e1b4b,stroke:#f59e0b,color:#f8fafc,stroke-width:2px;
    classDef env fill:#0f172a,stroke:#3b82f6,color:#f8fafc,stroke-width:2px;

    A["🤖 Agent (Decision Maker)"]:::agent -- "Action (a_t)" --> E["🌍 Environment (Simulator/Real World)"]:::env
    E -- "Reward (r_t+1) & Next State (s_t+1)" --> A
```
