...
```mermaid
flowchart TD
    classDef s3 fill:#1e293b,stroke:#475569,color:#f8fafc,stroke-width:2px;
    classDef lustre fill:#0369a1,stroke:#0ea5e9,color:#f8fafc,stroke-width:2px;
    classDef gpuStarve fill:#881337,stroke:#f43f5e,color:#f8fafc,stroke-width:2px;
    classDef gpuActive fill:#064e3b,stroke:#10b981,color:#f8fafc,stroke-width:2px;

    subgraph Bottleneck["❌ GPU Starvation Bottleneck"]
        S3["🗄️ Amazon S3 Data Lake"]:::s3 -- "🐢 Slow/Serial Read Stream<br/>(Network overhead)" --> GPU_S["📟 GPU Compute Node"]:::gpuStarve
        style GPU_S stroke-dasharray: 5 5
    end

    subgraph Optimal["✅ Parallel High-Throughput Mode"]
        FSx["⚡ FSx for Lustre (Cached S3)"]:::lustre -- "🚀 Parallel Fast I/O<br/>(Sub-millisecond latency)" --> GPU_A["📟 GPU Compute Node"]:::gpuActive
    end

    %% Force vertical stacking by linking Bottleneck to Optimal
    GPU_S -.->|" "| FSx
    linkStyle 2 stroke:none,stroke-width:0px,color:transparent;
```
...
```mermaid
graph TD
    classDef request fill:#1e293b,stroke:#475569,color:#f8fafc,stroke-width:2px;
    classDef real fill:#1e1b4b,stroke:#6366f1,color:#f8fafc,stroke-width:2px;
    classDef server fill:#0f172a,stroke:#0ea5e9,color:#f8fafc,stroke-width:2px;
    classDef async fill:#881337,stroke:#f43f5e,color:#f8fafc,stroke-width:2px;
    classDef batch fill:#064e3b,stroke:#10b981,color:#f8fafc,stroke-width:2px;

    S["🎯 Inference Request"]:::request --> Real["⚡ Real-Time Endpoint"]:::real
